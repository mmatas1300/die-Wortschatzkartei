...
function WorterbuchPage() {
    ...
    return (
        <div className="flex flex-row justify-center items-center flex-wrap mt-12">
            {alphaNum.map((x) => {
                return(
                    <Link key={x} href={`/worterbuch/${String.fromCharCode(x)}`} className={`${style.card} w-32 h-32 m-3 cursor-pointer text-base flex justify-center items-center rounded-xl transition duration-200 hover:scale-110`}>
